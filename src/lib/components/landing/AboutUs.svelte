<script lang="ts">
  ...
  function animateCounters() {
    ...
    const timer = setInterval(() => {
      ...
      countYears = Math.floor(easeProgress * 4);
...
<section id="who-we-are" class="w-full max-w-6xl px-6 py-20 z-10 border-t border-zinc-200/60">
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
    ...
    >
      <div
        class="w-72 h-72 rounded-3xl bg-gradient-to-tr from-[#ffaa00] via-[#ff3366] to-[#7928ca] p-8 flex flex-col justify-between shadow-2xl relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300"
      >
        <div class="absolute inset-0 bg-black/10"></div>
        <div
          class="z-10 w-14 h-14 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center border border-white/30"
        >
        ...
        <div class="z-10 text-white">
          ...
          <p class="text-xs font-black uppercase tracking-wider text-white/95">
            {$t('about_experience')}
          </p>
        </div>
      </div>
      <div
        class="absolute -inset-4 border border-dashed border-zinc-400/30 rounded-3xl -z-10 transform rotate-3"
      ></div>
    </div>
    ...
    >
      <div>
        <span
          class="text-xs font-black text-zinc-600 uppercase tracking-widest px-3.5 py-1 bg-white rounded-full border border-zinc-200 shadow-sm"
          >{$t('about_badge')}</span
        >
      </div>
      ...
      <div
        bind:this={statsContainerRef}
        class="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-6 border-t border-zinc-200/80"
      >
        <div
          class="bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-zinc-200/80 shadow-sm text-center"
        >
          <span
            class="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#ff007f] to-[#7928ca]"
            >{countYears}+</span
          >
        ...
        >
          <span
            class="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#7928ca] to-[#0070f3]"
            >{countVideos}+</span
          >
        ...
        >
          <span
            class="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#0070f3] to-[#00abbd]"
            >{countClients}+</span
          >
        ...
        >
          <span
            class="text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#00abbd] to-emerald-500"
            >{countSatisfaction}%</span
          >
